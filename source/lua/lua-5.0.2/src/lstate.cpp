/*
** $Id: lstate.cpp,v 1.1.1.1 2005/05/26 15:18:05 kajala Exp $
** Global State
...
static void f_luaopen (lua_State *L, void *ud) {
  ...
  global_State *g = luaM_new(0, global_State);
  UNUSED(ud);
  if (g == 0) luaD_throw(L, LUA_ERRMEM);
  L->l_G = g;
  g->mainthread = L;
...
static void close_state (lua_State *L) {
  ...
  if (G(L)) {  /* close global state */
    ...
    lua_assert(G(L)->rootgc == 0);
    lua_assert(G(L)->rootudata == 0);
    luaS_freeall(L);
...
LUA_API void lua_close (lua_State *L) {
  ...
  do {  /* repeat until no more errors */
    ...
    L->nCcalls = 0;
  } while (luaD_rawrunprotected(L, callallgcTM, 0) != 0);
  lua_assert(G(L)->tmudata == 0);
  close_state(L);
}
