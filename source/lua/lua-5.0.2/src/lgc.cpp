/*
** $Id: lgc.cpp,v 1.1.1.1 2005/05/26 15:18:05 kajala Exp $
** Garbage Collector
...
size_t luaC_separateudata (lua_State *L) {
  ...
  GCObject *curr;
  GCObject *collected = 0;  /* to collect udata with gc event */
  ...
  while ((curr = *p) != 0) {
    ...
    if (ismarked(curr) || isfinalized(gcotou(curr)))
    ...
    else if (fasttm(L, gcotou(curr)->uv.metatable, TM_GC) == 0) {
      ...
      *p = curr->gch.next;
      curr->gch.next = 0;  /* link `curr' at the end of `collected' list */
      *lastcollected = curr;
...
static void traversetable (GCState *st, Table *h) {
  ...
  if (mode && ttisstring(mode)) {  /* is there a weak mode? */
    weakkey = (strchr(svalue(mode), 'k') != 0);
    weakvalue = (strchr(svalue(mode), 'v') != 0);
