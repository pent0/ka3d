/*
** $Id: lvm.cpp,v 1.1.1.1 2005/05/26 15:18:06 kajala Exp $
** Lua virtual machine
...
static const TObject *luaV_index (lua_State *L, const TObject *t,
  ...
  if (tm == 0) return &luaO_nilobject;  /* no TM */
  if (ttisfunction(tm)) {
  ...
  do {
    if (ttistable(t)) {  /* `t' is a table? */
      ...
      if (!ttisnil(oldval) ||  /* result is no nil? */
          (tm = fasttm(L, h->metatable, TM_NEWINDEX)) == 0) { /* or no TM? */
...
static const TObject *get_compTM (lua_State *L, Table *mt1, Table *mt2,
  ...
  const TObject *tm2;
  if (tm1 == 0) return 0;  /* no metamethod */
  if (mt1 == mt2) return tm1;  /* same metatables => same metamethods */
  tm2 = fasttm(L, mt2, event);
  if (tm2 == 0) return 0;  /* no metamethod */
...
static void Arith (lua_State *L, StkId ra,
  ...
  TObject tempb, tempc;
  const TObject *b, *c;
  if ((b = luaV_tonumber(rb, &tempb)) != 0 &&
      (c = luaV_tonumber(rc, &tempc)) != 0) {
    switch (op) {
