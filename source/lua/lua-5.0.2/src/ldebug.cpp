/*
** $Id: ldebug.cpp,v 1.1.1.1 2005/05/26 15:18:05 kajala Exp $
** Debug Interface
...
LUA_API int lua_sethook (lua_State *L, lua_Hook func, int mask, int count) {
  if (func == 0 || mask == 0) {  /* turn off hooks? */
    mask = 0;
    func = 0;
  }
  L->hook = func;
...
static Proto *getluaproto (CallInfo *ci) {
  return (isLua(ci) ? ci_func(ci)->l.p : 0);
}
...
static int auxgetinfo (lua_State *L, const char *what, lua_Debug *ar,
  ...
  for (; *what; what++) {
    switch (*what) {
      ...
      case 'n': {
        ar->namewhat = (ci) ? getfuncname(ci, &ar->name) : 0;
        if (ar->namewhat == 0) {
          /* try to find a global name */
          if ((ar->name = travglobals(L, f)) != 0)
            ar->namewhat = "global";
...
LUA_API int lua_getinfo (lua_State *L, const char *what, lua_Debug *ar) {
  ...
  if (*what == '>') {
    ...
    status = auxgetinfo(L, what + 1, ar, f, 0);
    L->top--;  /* pop function */
  }
...
static const char *getfuncname (CallInfo *ci, const char **name) {
  ...
  if ((isLua(ci) && ci->u.l.tailcalls > 0) || !isLua(ci - 1))
    return 0;  /* calling function is not Lua (or is unknown) */
  ci--;  /* calling function */
  i = ci_func(ci)->l.p->code[currentpc(ci)];
  if (GET_OPCODE(i) == OP_CALL || GET_OPCODE(i) == OP_TAILCALL)
    return getobjname(ci, GETARG_A(i), name);
  else
    return 0;  /* no useful name can be found */
}
...
void luaG_typeerror (lua_State *L, const TObject *o, const char *op) {
  ...
  const char *kind = (isinstack(L->ci, o)) ?
                         getobjname(L->ci, o - L->base, &name) : 0;
  if (kind)
