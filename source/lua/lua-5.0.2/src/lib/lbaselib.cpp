/*
** $Id: lbaselib.cpp,v 1.1.1.1 2005/05/26 15:18:06 kajala Exp $
** Basic library
...
static int luaB_loadfile (lua_State *L) {
  const char *fname = luaL_optstring(L, 1, 0);
...
static const char *pushnextpath (lua_State *L, const char *path) {
  const char *l;
  if (*path == '\0') return 0;  /* no more paths */
  if (*path == LUA_PATH_SEP) path++;  /* skip separator */
  l = strchr(path, LUA_PATH_SEP);  /* find next separator */
  if (l == 0) l = path+strlen(path);
...
static void pushcomposename (lua_State *L) {
  ...
  const char *wild;
  int n = 1;
  while ((wild = strchr(path, LUA_PATH_MARK)) != 0) {
...
static int luaB_require (lua_State *L) {
  ...
  if (lua_toboolean(L, -1))  /* is it there? */
    ...
    while (status == LUA_ERRFILE) {
      ...
      if ((path = pushnextpath(L, path)) == 0) break;
      pushcomposename(L);
...
static void base_open (lua_State *L) {
  ...
  luaL_openlib(L, 0, base_funcs, 0);  /* open lib into global table */
