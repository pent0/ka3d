/*
** $Id: lauxlib.cpp,v 1.1.1.1 2005/05/26 15:18:06 kajala Exp $
...
LUALIB_API void *luaL_checkudata (lua_State *L, int ud, const char *tname) {
  const char *tn;
  if (!lua_getmetatable(L, ud)) return 0;  /* no metatable? */
...
static const char *getF (lua_State *L, void *ud, size_t *size) {
  LoadF *lf = (LoadF *)ud;
  (void)L;
  if (feof(lf->f)) return 0;
  *size = fread(lf->buff, 1, LUAL_BUFFERSIZE, lf->f);
  return (*size > 0) ? lf->buff : 0;
}
...
LUALIB_API int luaL_loadfile (lua_State *L, const char *filename) {
  ...
  if (lf.f == 0) return errfile(L, fnameindex);  /* unable to open file */
  c = ungetc(getc(lf.f), lf.f);
  if (!(isspace(c) || isprint(c)) && lf.f != stdin) {  /* binary file? */
    fclose(lf.f);
    lf.f = fopen(filename, "rb");  /* reopen in binary mode */
    if (lf.f == 0) return errfile(L, fnameindex); /* unable to reopen file */
  }
