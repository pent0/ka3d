/*
** $Id: lstrlib.cpp,v 1.1.1.1 2005/05/26 15:18:06 kajala Exp $
  ...
  switch (*p) {
    ...
    case '$': {
      if (*(p+1) == '\0')  /* is the `$' the last char in pattern? */
        return (s == ms->src_end) ? s : 0;  /* check end of string */
      else goto dflt;
    }
    default: dflt: {  /* it is a pattern item */
      ...
      switch (*ep) {
        ...
        case '+': {  /* 1 or more repetitions */
          return (m ? max_expand(ms, s+1, p, ep) : 0);
        }
...
static const char *lmemfind (const char *s1, size_t l1,
  ...
  else if (l2 > l1) return 0;  /* avoids a negative `l1' */
  else {
...
static int str_find (lua_State *L) {
  ...
  if (lua_toboolean(L, 4) ||  /* explicit request? */
      strpbrk(p, SPECIALS) == 0) {  /* or no special characters? */
    /* do a plain search */
