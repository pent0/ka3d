/*
** $Id: liolib.cpp,v 1.1.1.1 2005/05/26 15:18:06 kajala Exp $
...
static FILE **topfile (lua_State *L, int findex) {
  ...
  if (f == 0) luaL_argerror(L, findex, "bad file");
  return f;
}
...
static int aux_close (lua_State *L) {
  ...
  if (f == stdin || f == stdout || f == stderr)
    ...
    if (ok)
      *(FILE **)lua_touserdata(L, 1) = 0;  /* mark file as closed */
    return ok;
  }
}
...
static int io_close (lua_State *L) {
  ...
  return pushresult(L, aux_close(L), 0);
}
...
static int io_tmpfile (lua_State *L) {
  FILE **pf = newfile(L);
  *pf = tmpfile();
  return (*pf == 0) ? pushresult(L, 0, 0) : 1;
}
...
static int read_line (lua_State *L, FILE *f) {
  ...
  for (;;) {
    ...
    if (fgets(p, LUAL_BUFFERSIZE, f) == 0) {  /* eof? */
...
static int io_flush (lua_State *L) {
  return pushresult(L, fflush(getiofile(L, IO_OUTPUT)) == 0, 0);
}


static int f_flush (lua_State *L) {
  return pushresult(L, fflush(tofile(L, 1)) == 0, 0);
}
...
static int io_getenv (lua_State *L) {
  lua_pushstring(L, getenv(luaL_checkstring(L, 1)));  /* if 0 push nil */
  return 1;
}
...
static int io_time (lua_State *L) {
  if (lua_isnoneornil(L, 1))  /* called without args? */
    lua_pushnumber(L, time(0));  /* return current time */
  else {
    time_t t;
    struct tm ts;
...
LUALIB_API int luaopen_io (lua_State *L) {
  ...
  registerfile(L, stderr, "stderr", 0);
  return 1;
}
