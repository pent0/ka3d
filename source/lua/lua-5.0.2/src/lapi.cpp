/*
** $Id: lapi.cpp,v 1.1.1.1 2005/05/26 15:18:05 kajala Exp $
** Lua API
...
LUA_API int lua_isuserdata (lua_State *L, int idx) {
  const TObject *o = luaA_indexAcceptable(L, idx);
  return (o != 0 && (ttisuserdata(o) || ttislightuserdata(o)));
}
...
LUA_API int lua_rawequal (lua_State *L, int index1, int index2) {
  ...
  return (o1 == 0 || o2 == 0) ? 0  /* index out of range */
...
LUA_API int lua_equal (lua_State *L, int index1, int index2) {
  ...
  i = (o1 == 0 || o2 == 0) ? 0  /* index out of range */
                                 : equalobj(L, o1, o2);
  lua_unlock(L);
  return i;
}
...
LUA_API const char *lua_tostring (lua_State *L, int idx) {
  ...
  if (o == 0)
    ...
    s = (luaV_tostring(L, o) ? svalue(o) : 0);
    luaC_checkGC(L);
    lua_unlock(L);
    return s;
  }
...
LUA_API lua_CFunction lua_tocfunction (lua_State *L, int idx) {
  StkId o = luaA_indexAcceptable(L, idx);
  return (o == 0 || !iscfunction(o)) ? 0 : clvalue(o)->c.f;
}
...
LUA_API lua_State *lua_tothread (lua_State *L, int idx) {
  StkId o = luaA_indexAcceptable(L, idx);
  return (o == 0 || !ttisthread(o)) ? 0 : thvalue(o);
}
...
LUA_API int lua_getmetatable (lua_State *L, int objindex) {
  ...
  if (mt == 0 || mt == hvalue(defaultmeta(L)))
    res = 0;
  else {
    sethvalue(L->top, mt);
    api_incr_top(L);
...
LUA_API void lua_concat (lua_State *L, int n) {
  ...
  if (n >= 2) {
    ...
    setsvalue2s(L->top, luaS_newlstr(L, 0, 0));
    api_incr_top(L);
  }
...
static const char *aux_upvalue (lua_State *L, int funcindex, int n,
  ...
  if (!ttisfunction(fi)) return 0;
  f = clvalue(fi);
  if (f->c.isC) {
    if (n > f->c.nupvalues) return 0;
    *val = &f->c.upvalue[n-1];
    return "";
  }
  else {
    Proto *p = f->l.p;
    if (n > p->sizeupvalues) return 0;
