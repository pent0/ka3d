/*
** $Id: ltable.cpp,v 1.1.1.1 2005/05/26 15:18:05 kajala Exp $
** Lua tables (hash)
...
static void setnodevector (lua_State *L, Table *t, int lsize) {
  ...
  if (lsize == 0) {  /* no elements to hash part? */
    ...
    lua_assert(t->node->next == 0);  /* (`dummynode' must be empty) */
  }
  else {
...
static void resize (lua_State *L, Table *t, int nasize, int nhsize) {
  ...
  if (oldhsize)
    ...
    lua_assert(G(L)->dummynode->next == 0);
  }
